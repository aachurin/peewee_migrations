...
class Router:
    ...
    def create(self, name=None, tracecode=None):
        ...
        migrator = Migrator(
            self.database, name, last_snap, new_snap, compute_hints=True
        )
        migrator.migrate()
...
class Migrator:
    ...
    def __init__(
        ...
        if compute_hints:
            ...
            self.backward_hints = MigrateCode(
                "backward", ("old_orm", "new_orm")
            )
        else:
            self.compute_hints = False
        self.op = OperationProxy(
            Operations.from_database(database), self.add_op
        )
        ...
        if self.run_data_migration:
            ops = list(
                self.run_data_migration(
                    new_orm=self.new_orm, old_orm=self.old_orm
                )
            )
    ...
    def _render_migrate_state(self, model1, model2):
        ...
        return State(
            ...
            drop_constraints=[
                constaints1[key] for key in set(constaints1) - set(constaints2)
            ],
            add_constraints=[
                constaints2[key] for key in set(constaints2) - set(constaints1)
            ],
...
class OperationProxy:
    ...
    def __getattr__(self, attr):
        ...
        if hasattr(obj, "__func__") and getattr(
            obj.__func__, "is_operation", False
        ):
...
class Operations:
    ...
    def from_database(cls, database):
        ...
        # init base Operations class for sqlite; can be further developed
    ...
    def drop_column(self, model: peewee.Model, field: peewee.Field):
        return self._alter_table(model).literal(" DROP COLUMN ").sql(field)

    @operation
    @model_field_args
    def rename_column(
        self, model: peewee.Model, field: peewee.Field, column_name: str
    ):
        return self._rename_column(
            model, field, field.column_name, column_name
        )
    ...
    def drop_foreign_key_constraint(
        self, model: peewee.Model, field: peewee.Field
    ):
...
class PostgresqlOperations(Operations):
    ...
    def _drop_primary_key_constraint(self, model):
        params = (model._meta.table_name, model._meta.schema or "public")
...
class MySQLOperations(Operations):
    ...
    def _drop_primary_key_constraint(self, model: peewee.Model):
        ...
        operations.append(
            self._alter_table(model).literal(" DROP PRIMARY KEY")
        )
        return operations
